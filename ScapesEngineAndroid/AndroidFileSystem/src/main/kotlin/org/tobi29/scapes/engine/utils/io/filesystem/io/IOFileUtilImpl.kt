...
class IOFileUtilImpl : FileUtilImpl {
    ...
    override fun <R> stream(path: FilePath,
        ...
        return consumer(list(path).stream())
    }

    override fun list(path: FilePath): List<FilePath> {
        val list = ArrayList<FilePath>()
        val files = toFile(path).listFiles()
        if (files != null) {
            for (child in files) {
                list.add(path(child))
            }
    ...
    override fun <R> tempChannel(path: FilePath,
        ...
        val file = toFile(path)
        val ret = channelRW(file).use { consumer(it) }
    ...
    private data class FilePathImpl(val file: File) : FilePath {
        ...
        override fun relativize(other: FilePath): FilePath? {
            return file.relativeToOrNull(toFile(other))?.mapNotNull { path(it) }
        }
    ...
    companion object {
        ...
        fun read(file: File): ReadSource {
            return object : ReadSource {
                ...
                override fun <R> read(reader: (ReadableByteStream) -> R): R {
                    return IOFileUtilImpl.read(file, reader)
                }
        ...
        private fun listRecursive(file: File,
                                  list: MutableList<FilePath>,
                                  vararg filters: (FilePath) -> Boolean) {
            val files = file.listFiles()
            if (files != null) {
                for (child in files) {
                    val filePath = path(child)
