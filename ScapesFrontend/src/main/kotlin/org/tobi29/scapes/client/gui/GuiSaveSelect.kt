...
class GuiSaveSelect(
    ...
    style: GuiStyle
) : GuiMenuDouble(state, "Singleplayer", "Create", "Back", previous, style) {
    ...
    init {
        ...
        save.on(GuiEvent.CLICK_LEFT) { event ->
            launch(engine.taskExecutor) {
                try {
                    val plugins = Plugins.available().map { it.first }
                    val worldTypes = plugins.asSequence()
                        .filter { it.parent == "WorldType" }
                        .sortedBy { it.name }.toList()
                    if (worldTypes.isEmpty()) {
                        state.engine.guiStack.swap(
                            this@GuiSaveSelect,
                            GuiMessage(
                                state, this@GuiSaveSelect, "Error",
                                NO_WORLD_TYPE, style
                            )
                        )
                    } else {
