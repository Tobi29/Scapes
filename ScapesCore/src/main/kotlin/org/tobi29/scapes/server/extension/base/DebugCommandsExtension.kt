...
class DebugCommandsExtension(server: ScapesServer) : ServerExtension(server) {
    ...
    override fun init() {
        ...
        group.register("give", 8) {
            ...
            val materialOption = commandOption(
                setOf('m'), setOf("material"), listOf("name"),
                "Material of item"
            )
            ...
            val amountOption = commandOption(
                setOf('a'), setOf("amount"), listOf("value"),
            ...
            return@register { args, executor, commands ->
                ...
                commands.add {
                    ...
                    val material = requireGet(
                        { plugins.materialResolver[it] },
                        materialName
                    )
                    val item = Item(
                        material,
        ...
        group.register("tp", 8) {
            ...
            val targetOption = commandOption(
                ...
                "Target that will be teleported to"
            )
            val worldOption = commandOption(
                setOf('w'), setOf("world"), listOf("name"),
                "World that will be teleported to"
            )
            val locationOption = commandOption(
                setOf('l'), setOf("location"), listOf("x", "y", "z"),
                "Location that will be teleported to"
            )
            return@register { args, executor, commands ->
                val playerName = args.require(
                    playerOption
        ...
        group.register("item", 8) {
            val playerOption = commandOption(
                setOf('p'), setOf("player"), listOf("name"),
                "Player holding the item in the left hand"
            )
            return@register { args, executor, commands ->
                val playerName = args.require(
                    playerOption
