...
abstract class TextureAtlas<T : TextureAtlasEntry> protected constructor(val engine: ScapesEngine, minBits: Int) {
    ...
    fun init(engine: ScapesEngine): Int {
        ...
        while (y < size) {
            ...
            while (x < size) {
                ...
                if (i > 0) {
                    ...
                    val scansize = texture.resolution shl 2
                    var row = 0
                    texture.buffer?.let { textureBuffer ->
                        while (row < texture.resolution) {
                            buffer.position((yy + row) * imageSize + xx shl 2)
                            row++
                            textureBuffer.limit(scansize * row)
        ...
        buffer.rewind()
        texture = engine.graphics.createTexture(imageSize, imageSize, buffer, 4)
        sources.clear()
