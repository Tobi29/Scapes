...
public final class TreeOak implements Tree {
    public static final TreeOak INSTANCE = new TreeOak();
    ...
    public void gen(TerrainServer.TerrainMutable terrain, int x, int y, int z,
        ...
        for (int i = 0; i < random.nextInt(4) + 4 * trunkSize; i++) {
            branches.add(new Vector3i(random.nextInt(7) - 3 + x,
                    random.nextInt(7) - 3 + y,
                    random.nextInt(2) + 2 + z + size));
        }
        for (int i = 0; i < random.nextInt(3) + trunkSize; i++) {
        ...
        for (Vector3 branch : branches) {
            ...
            TreeUtil.makeLeaves(terrain, branch.intX(), branch.intY(),
                    branch.intZ(), materials.leaves, (short) 0, 6);
        }
    }
}
