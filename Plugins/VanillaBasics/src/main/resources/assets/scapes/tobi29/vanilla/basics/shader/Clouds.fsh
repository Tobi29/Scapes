...
void main(){
    vec2 pos = varying_Texture + uniform_CamPos;
    float n = min(abs(simplexNoise3(vec3(pos.x * 10.0, pos.y * 10.0, uniform_Time)) * 1.5 + simplexNoise3(vec3(pos.x * 200.0, pos.y * 200.0, uniform_Time)) * 0.5), 1.0);
    float a = clamp(simplexNoise3(vec3(pos.x * 3.0, pos.y * 3.0, uniform_Time)) - 0.9 + uniform_Weather * 3.0 - n * 0.02, 0.0, 1.0);
    out_Color.a = clamp(a * 1.4, 0.0, 1.0);
    if (out_Color.a <= 0.01) {
        discard;
    }
    out_Color.rgb = vec3(clamp(2.0 - a * 1.6 - n * 0.1 - max(uniform_Weather * 2.0 - 1.0, 0.0), 0.0, 1.0));
}