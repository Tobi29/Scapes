...
class BlockOreGold(materials: VanillaMaterial,
                   stoneRegistry: GameRegistry.Registry<StoneType>) : BlockOre(
        materials, "vanilla.basics.block.OreGold", stoneRegistry) {
    override fun dropsOre(item: ItemStack,
                          data: Int): List<ItemStack> {
        return listOf(ItemStack(materials.oreChunk, 7))
    }

    override fun name(item: ItemStack) = "${stoneName(item)} Gold Ore"

    override fun oreTexture() = "Gold"
}
