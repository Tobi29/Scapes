...
public class BlockOreCassiterite extends BlockOre {
    public BlockOreCassiterite(VanillaMaterial materials,
            GameRegistry.Registry<StoneType> stoneRegistry) {
        super(materials, "vanilla.basics.block.OreCassiterite", stoneRegistry);
    }
    ...
    public boolean destroy(TerrainServer.TerrainMutable terrain, int x, int y,
        ...
        if ("Pickaxe".equals(item.getMaterial().getToolType(item)) &&
                !canBeBroken(item.getMaterial().getToolLevel(item),
                        terrain.data(x, y, z))) {
            terrain.world()
                    .dropItem(new ItemStack(materials.oreChunk, (short) 2),
    ...
    public List<ItemStack> getDrops(ItemStack item, int data) {
        if ("Pickaxe".equals(item.getMaterial().getToolType(item)) &&
            ...
            return Arrays.asList(new ItemStack(materials.oreChunk, (short) 2),
    ...
    public String getName(ItemStack item) {
        return getStoneName(item) + " Cassiterite Ore";
    }
