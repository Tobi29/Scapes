...
open class GuiInventory(
    name: String,
    protected val player: MobPlayerClientMainVB,
    style: GuiStyle
) : GuiMenuSingle(player.game, name, style) {
    ...
    init {
        ...
        for (y in 0..2) {
            ...
            for (x in 0..9) {
                buttons.add(row.addHori(5.0, 5.0, -1.0, -1.0) { button(it, i) })
                i++
            }
            buttons.clear()
        }
        inventoryPane.addVert(0.0, 0.0, -1.0, 10.0) {
            GuiComponentGroup(it)
        }
        i = 0
        val row = inventoryPane.addVert(0.0, 0.0, -1.0, -1.0) {
    ...
    protected fun leftClick(
        id: String,
        i: Int
    ) {
        player.connection().send(
            PacketInventoryInteraction(
    ...
    public override fun renderOverlay(
        gl: GL,
        shader: Shader,
        pixelSize: Vector2d
    ) {
        ...
        gl.matrixStack.push { matrix ->
            ...
            player.inventories.access("Hold") { inventory ->
                GuiUtils.items(
                    cursorX.toFloat(), cursorY.toFloat(), 60.0f,
